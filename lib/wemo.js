...
const SYNC_TIMEOUT = 5000 // mark other switches in a set as dead after 5s timeout
...
class Wemo {
    ...
    if (deviceInfo.seen) {
      if (!refresh && changed) {
        ...
        if (this.switches[deviceInfo.serialNumber]) {
          ...
          // previous sync successful, clear sync timeout
          if (deviceInfo.sync) {
            clearTimeout(deviceInfo.sync)
            deviceInfo.sync = null
            this.log.trace(`${name}: Alive, clearing sync timeout`)
          }
        }
  ...
  syncBinaryState(serialNumber, value, refresh) {
    ...
    // set binarystate
    client.setBinaryState(value, (err) => {
      if (err) {
        return this.log.error(`${otherName}: ${err.code} during syncBinaryState`)
      }
    })

    // ensure other switches respond in time, or mark them as dead
    if (deviceInfo.sync) clearTimeout(deviceInfo.sync)
    deviceInfo.sync = setTimeout(() => {
      // mark client as dead so wemo-client can rediscover/resubscribe
      deviceInfo.sync = null
      client.subscriptions['urn:Belkin:service:basicevent:1'] = null
      client.error = 'ETIMEDOUT'
      this.log.error(
        `${otherName}: Failed to set 3-way to ${value === '1' ? 'ON' : 'OFF'}, marking as dead`
      )
  ...
  refreshBinaryState(client, deviceInfo) {
    ...
    client.getBinaryState((err, value) => {
      if (err) {
        return this.log.trace(`${name}: ${err.code} during refreshBinaryState`)
      }
